...
def article_view(request,template_name = "blog/article.html",**kwargs):
	...
	if not article.created_by.username == request.user.username:
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if is_publishing:
				...
				if request.user not in article.users_like:
	...
	if request.method == "POST":
		...
		if article_create_form.is_valid():
			...
			if is_publishing:
				...
				# services.switch_like(article,user=request.user) # dont allow user to like by default
